...
minetest.register_on_joinplayer(function(player)
	auditory_hallucination(player, 4, 5, 4, 10, 0.1, 0.2)

	--HEALTH.check_for_effect(player, {"Meta-Stim", 1}, {{"Meta-Stim"}, {"Neurotoxicity", "Meta-Stim", 1},})
	HEALTH.check_for_effect(player, {"Hangover (mild)", 1}, {{"Hangover (mild)"}})

--[[
	stagger(player, 10, 20, 0.5, 5, 4)
	...
	local block = {
		{"Hangover (mild)"},
		{"Hangover (moderate)", "Hangover (mild)", 1},
		{"Hangover (severe)", "Hangover (mild)", 1},
	}
	HEALTH.remove_effect(meta, effects_list, "Hangover (moderate)", swap, block)
]]
