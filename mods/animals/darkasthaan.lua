...
--[[
prefers the dark. Put in caves. Creepy, major threat, minor food source.
]]
---------------------------------------------------------------------
local random = math.random
local energy_max = 10000 --secs it can survive without food
...
local function spider_brain(self)
	...
	if mobkit.timer(self,1) then
		...
		if prty < 20 then
			...
			local rival = mobkit.get_closest_entity(self,'animals:darkasthaan')
			if rival then
			...
			if not rival and energy >= energy_max and random() > 0.99 then
				...
				if minetest.get_node(p).name =='air' then
					...
					if n and n.walkable then
						minetest.set_node(p, {name = 'animals:darkasthaan_eggs'})
...
minetest.register_node("animals:darkasthaan_eggs", {
	...
	on_use = function(itemstack, user, pointed_thing)
		...
		return HEALTH.use_item(itemstack, user, 0, 1, 20, -20, 0)
	end,
	...
	on_timer =function(pos, elapsed)
		...
		if #air < 1 then
			...
			for _, wp in pairs(air) do
				if random() >= 0.8 then
					local ent = minetest.add_entity(wp,"animals:darkasthaan")
					minetest.sound_play("animals_hatch_egg", {pos = pos, gain = random(0.1,0.4), max_hear_distance = 8})
...
minetest.register_node("animals:dead_darkasthaan", {
	...
	on_use = function(itemstack, user, pointed_thing)
		--hp_change, thirst_change, hunger_change, energy_change, temp_change, replace_with_item
		return HEALTH.use_item(itemstack, user, 0, 2, 10, -10, 0)
	end,
})
